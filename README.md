...
### 2.2 Mermaid Architecture Diagram (Source)

The same architecture, represented as a Mermaid diagram:

```mermaid
flowchart TD
    %% Global Styles
    classDef client fill:#f9f,stroke:#333,stroke-width:2px;
    classDef compute fill:#4285F4,stroke:#333,stroke-width:2px,color:white;
    classDef storage fill:#DB4437,stroke:#333,stroke-width:2px,color:white;
    classDef queue fill:#F4B400,stroke:#333,stroke-width:2px,color:white;
    classDef db fill:#0F9D58,stroke:#333,stroke-width:2px,color:white;

    %% External Client
    Client([User / Streamlit UI])
    Webhook([Webhook Listener])
    class Client,Webhook client;
    ...
    subgraph GCP [Google Cloud Platform]
        ...
        subgraph Compute [Cloud Run Microservices]
            Gateway[API Gateway\n(FastAPI)]
            Orch[Orchestrator\n(Job Manager / FFmpeg)]
            Worker[GPU/CPU Worker\n(YOLOv8 + OCR)]
            class Gateway,Orch,Worker compute;
        end

        %% Data Flow Connections
        Client -->|"1. POST /video + File"| Gateway
        Gateway -->|"2. Upload Original"| GCS
        Gateway -->|"3. Create Job (QUEUED)"| Firestore
        Gateway -->|"4. Dispatch Task"| Tasks

        Tasks -->|"5. Trigger /ingest"| Orch
        Orch -->|"6. Download & Analyze"| GCS
        Orch -->|"7. FFmpeg Split (if > 5min)"| Orch
        Orch -->|"8. Upload Chunks"| GCS
        Orch -->|"9. Update Job (CHUNKING)"| Firestore
        Orch -->|"10. Dispatch Chunk Tasks"| Tasks

        Tasks -->|"11. Trigger /process-chunk"| Worker
        Worker -->|"12. Fetch Config & Profile"| Firestore
        Worker -->|"13. Download Chunk"| GCS
        Worker -->|"14. Inference (Detection + Redaction)"| Worker
        Worker -->|"15. Upload Redacted Chunk"| GCS
        Worker -->|"16. Update Progress"| Firestore

        Worker -.->|"17. Trigger /stitch (When Done)"| Orch
        Orch -->|"18. Download Processed Chunks"| GCS
        Orch -->|"19. FFmpeg Concat + Strip Metadata"| Orch
        Orch -->|"20. Upload Final Video"| GCS
        Orch -->|"21. Mark COMPLETED"| Firestore
    end

    Orch -->|"22. Notification POST"| Webhook
    Client -.->|"Poll Status / Download"| Gateway
```
